...
- Full-screen - the player can enter fullscreen by pushing the F button and leave fullscreen with the Esc button.
...
Heavy optimizing is not implemented since the game runs very fast on its own. There is a dictionary implemented in the Grid class in order to get the correct number picture based on the number of adjacent bombs a tile has.

A seperate class is used to represent the scores in the _Leaderboards_ menu, the achievements in the _Achievements_ menu and another one for serializing the best scores and unlocked achievements. 
